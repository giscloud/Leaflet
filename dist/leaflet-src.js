...
L.LatLng = function (/*Number*/ rawLat, /*Number*/ rawLng, /*Boolean*/ noWrap) {
	...
	/*
      - wrapping can break the GIS Cloud projection system
	if (noWrap !== true) {
...
L.TileLayer = L.Class.extend({
	...
	_initContainer: function () {
		...
		if (!this._container || tilePane.empty) {
			...
			if (this._oldContainer) {
				tilePane.insertBefore(this._container, this._oldContainer.nextSibling);
			} else if (this._insertAtTheBottom && first) {
...
L.Map.TouchZoom = L.Handler.extend({
	...
	_onTouchMove: function (e) {
		...
		var id, layer;
		for (id in this._map._layers) {
			if (this._map._layers.hasOwnProperty(id)) {
				...
				if (L.TileLayer && (layer instanceof L.TileLayer) && layer.getVisible() &&
					...
					layer._container.style.webkitTransform = [
...
L.Map.Measure = L.Handler.extend({
	...
	_stretchElasticLine: function (e) {
		...
		if (this._elasticLineTipMarker) {
			...
			this._elasticLineTipMarker.setLabelText(
